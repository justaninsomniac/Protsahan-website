jQuery(document).ready(function($){
	...
	function animateSection () {
		...
		$('.cd-section').each(function(){
			...
			( offset >= -200 && offset < windowHeight ) ? actualBlock.addClass('is-visible') : actualBlock.removeClass('is-visible');		
		});
	}
	...
	function prevSection() {
		if (!animating) {
			...
			if(prevSection.length > 0 && $(window).scrollTop() != prevSection.offset().top && prevSection.offset().top>200) {
				smoothScroll(prevSection);
			} else if(prevSection.prev().length > 0 && $(window).scrollTop() == prevSection.offset().top && prevSection.offset().top>200) {
